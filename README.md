...
ESP32-S3 보드가 CSI를 수집하고, Mac이 JSONL로 저장하며, 후처리 파이프라인이
LSTM 학습 텐서를 생성합니다. 수집 경로는 2가지입니다:

- **USB 수집** — RX 보드를 USB로 연결해 시리얼로 100Hz 수집 (모델 학습 데이터 표준 경로)
- **AP 실시간 수집** — TX SoftAP + RX UDP 무선 전송
...
```text
TX ESP-NOW 10ms → RX CSI 100Hz → Mac JSONL → (N, 300, RX수×52) 텐서 → LSTM
```
