...
Early stopping은 validation macro-F1이 더 이상 좋아지지 않을 때 최대 50 epoch를
...
Patience 값은 학습 코드를 구현할 때 config에 명시한다. 이 값은 train과
validation만 보고 정한 뒤 첫 test 전에 고정하며, run 산출물에 기록해 같은 종료
조건을 재현할 수 있게 한다. Test 결과가 좋지 않다는 이유로 patience를 바꾸지
않는다.

비교 seed `0`, `1`, `2`는 class나 session 번호가 아니라 모델 초기값, train
shuffle 순서, Dropout mask 등의 무작위 시작점을 정하는 번호다. 같은 데이터와
설정으로 seed만 바꿔 세 번 학습하고, seed마다 validation macro-F1이 가장 높은
...
구체적으로는 같은 split과 모델 설정으로 weight 없는 baseline과 weight 적용
모델을 각각 학습한 뒤 validation 결과를 비교한다. Class별 recall은 실제 해당
class 중 올바르게 찾은 비율이다.

```text
class c recall = 실제 class c를 class c로 맞힌 수 / 실제 class c 전체 수
```

Weight 적용 후 validation macro-F1과 부족했던 class의 recall이 개선되고 다른
class의 recall이 크게 무너지지 않으면 적용한다. Weight는 예를 들어 motion
오분류 loss에 `1.13`을 곱해 파라미터 갱신에서 조금 더 중요하게 다루는 값이다.
입력 `X`, `mean`, `std_safe`는 두 실험에서 동일하다. 이 선택은 validation으로
끝내고, 확정된 한 방식만 test에서 최종 평가한다.
...
Validation 전체의 macro-F1이 이전 최고점보다 높으면 현재 파라미터를 best
...
macro-F1이 가장 높았던 best checkpoint를 사용한다.
